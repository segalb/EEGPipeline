...
function [outvar1] = NCL_eegplot(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
    ...
    % ---------------------------------
    defdowncom   = 'NCL_eegplot(''defdowncom'',   gcbf);'; % push button: create/remove window
    defmotioncom = 'NCL_eegplot(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
    defupcom     = 'NCL_eegplot(''defupcom'',     gcbf);';
    defctrldowncom = 'NCL_eegplot(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
    ...
    try, g.title; 		    catch, g.title		= ['Scroll activity -- NCL_eegplot()']; 	end;
    ...
    if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
        ...
        NCL_eegplot('setelect', g.eloc_file, ax1);
    end;
    ...
    if ~isempty(g.events)
        u(17) = uicontrol('Parent',figh, ...
            'Units', 'normalized', ...
            'Position',posbut(17,:), ...
            'string', 'Event types', 'callback', 'NCL_eegplot(''drawlegend'', gcbf)');
    end;
    ...
    uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'NCL_eegplot(''noui'');');
    ...
    % Scale Eye %%%%%%%%%
    timestring = ['[OBJ1,FIG1] = gcbo;',...
        'NCL_eegplot(''scaleeye'',OBJ1,FIG1);',...
        'clear OBJ1 FIG1;'];
    m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
    
    % Title %%%%%%%%%%%%
    uimenu('Parent',m(1),'Label','Title','Callback','NCL_eegplot(''title'')')
    ...
    % Threshold Windows %%%%%%%%%%%%%%%%%%%
    threshmenu = uimenu('Parent',m(1),'Label','Thresholds');
    uimenu('Parent',threshmenu,'Label','View Channel/Epoch Thresholds','Callback','plotthresh.surface');
    uimenu('Parent',threshmenu,'Label','Change Thresholds & View Epoch Rejection','Callback','changethresh.surface');
    ...
    uimenu('Parent',m(2),'Label','Time range to display',...
        'Callback','NCL_eegplot(''window'')')
    
    % Electrode window %%%%%%%%
    uimenu('Parent',m(2),'Label','Number of channels to display',...
        'Callback','NCL_eegplot(''winelec'')')
    ...
    uimenu('Parent',m(6),'Label','Load .loc(s) file',...
        'Callback','NCL_eegplot(''loadelect'');')
    
    % Zooms %%%%%%%%
    zm = uimenu('Parent',m(2),'Label','Zoom off/on');
    commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); NCL_eegplot(''''zoom'''', gcbf, 1);'' ]);' ...
    ...
    complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''on'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; NCL_eegplot(''drawp'', 0);'];
    comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''off'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; NCL_eegplot(''drawp'', 0);'];
    comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''on'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; NCL_eegplot(''emaxstring'');']; % JavierLC
    comeventleg  = [ 'NCL_eegplot(''drawlegend'', gcbf);'];
    ...
    NCL_eegplot('scaleeye', [], gcf);
    if strcmp(lower(g.scale), 'off')
        NCL_eegplot('scaleeye', 'off', gcf);
    end;
    
    NCL_eegplot('drawp', 0);
    NCL_eegplot('drawp', 0);
    if g.dispchans ~= g.chans
        NCL_eegplot('zoom', gcf);
    end;
    NCL_eegplot('scaleeye', [], gcf);
    ...
    switch data
        case 'drawp' % Redraw EEG and change position
            ...
            if ~isempty(changeEpochNum)
                set(EPosition,'string',num2str(changeEpochNum));
            end
            clearvars -global changeEpochNum;
            ...
            % Update Filter Thresholds Window - Added by David DiStefano, 11/10/16
            % -------------------------------
            plotthresh.update(str2num(EPosition.String));
            ...
            if g.children ~= 0
                ...
                NCL_eegplot( 'drawp', p1, p2, g.children);
                figure(figh);
            end;
        ...
        case 'winelec'  % change channel window size
            ...
            set(gcf, 'UserData', g);
            NCL_eegplot('updateslider', fig);
            NCL_eegplot('drawp',0);
            NCL_eegplot('scaleeye', [], fig);
            return;
        ...
        case 'loadelect' % load channels
            ...
            NCL_eegplot('setelect',[ inputpath inputname ],AXH0);
            return;
        ...
        case 'zoom' % if zoom
            ...
            % ------------------------------
            if exist('p2', 'var') == 1
                set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); NCL_eegplot(''zoom'', gcbf, 1);' ]);
            ...
            if strcmp(get(fig, 'SelectionType'),'normal');
                ...
                if g.incallback ~= 1 % interception of nestest calls
                    ...
                    if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                        ...
                        set(fig,'UserData', g);
                        NCL_eegplot('drawp', 0);  % redraw background
                    end;
                end;
        ...
        otherwise
            error(['Error - invalid NCL_eegplot() parameter: ',data])
    end
end
...
if  all([tmppos(1,1) >= 0,tmppos(1,2)>= 0])
    ...
    if g.incallback
        ...
        if g.trialstag ~= -1,
            ...
            % Update threshold table figure --- Added by David DiStefano, 11/10/16
            plotthresh.update(g.time+ceil(mod(tmppos(1)/g.trialstag,5))); %update figure data to epoch that is hovered over in EEGPLOT
        else
