...
if nargin==1
        ...
        [EEG, com] = pop_artflatline(EEG, 'Twindow', testwindow, 'Threshold', ampth,...
                'Duration', dur, 'Channel', chanArray, 'Flag', flag, 'Review', viewstr, 'History', 'gui');
        return
end
...
clearvars -global outflat outflatcounts;
global outflat outflatcounts;  %added by EW 03/23/2015 to get values
outflatcounts = 0;
for ch=1:nch
        fprintf('%g ',chanArray(ch));
        for i=1:ntrial
                dataline = EEG.data(chanArray(ch), p1:p2 ,i);
                [captured,value] = NCL_flatline(dataline, ampth, dursam);
