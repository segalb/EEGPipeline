...
function plottopo_II_bss_biosemi(data, varargin)
    
%
%%%%%%%%%%%%%%%%%%%%% Graphics Settings - can be customized %%%%%%%%%%%%%%%%%%
%
LINEWIDTH     = 2;     % data line widths (can be non-integer)
...
%
  ...
  if ~isempty(g.geom)
    ...
    if plotgrid
        ...
        xvals=[-.5 0 -.3 -1 -.5 -.25 -.75 -1 -.5 -.45 -.9 -1   -.5  0    -.5  0    .5   .5   1    0.9  .45  0 0  .5 1  .75 .25 0   .5 1  0.3 .5 -1 1  xvals(35) 1    xvals(37:38)];
        yvals=[.6 .6 0.45 .3 .3  .15   .15 0  0 -.15 -.15 -.3 -.3 -.3 -.45 -.45 -.45 -.3 -.3 -.15 -.15 -.15 0 0 0  .15 .15 .3 .3 .3 0.45 .6 .6 .6 yvals(35) -.45 yvals(37:38)];
        
        %BEN'S ORIGINAL PLOT COORDINATES EXCLUDING AF3, AF4 and CPz
        % xvals=[-.5 0 xvals(3) -1 -.5 -.25 -.75 -1 -.5 -.25 -.75 -1   -.5  0    -.5  0    .5   .5   1    .75  .25  xvals(22) 0  .5 1  .75 .25 0   .5 1  yvals(31) .5 -1 1  xvals(35) 1    xvals(37:38)];
        % yvals=[.6 .6 yvals(3) .4 .4  .25   .25 .1  .1 -.05 -.05 -.25 -.25 -.25 -.45 -.45 -.45 -.25 -.25 -.05 -.05 yvals(22) .1 .1 .1  .25 .25 .4 .4 .4 yvals(31) .6 .6 .6 yvals(35) -.45 yvals(37:38)];
    ...
    for P=0:datasets-1, %  for each data epoch
        ...
        for c=1:chans, %%%%%%%% for each data channel %%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if P == datasets-1 % last pass
                               ...
                               %%%%%%%%%%%%%%%%%%%%%%% Plot lines %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                               %
                ymin_plot = -2; %for the plotting of the vertical axis (not the scale)
                ymax_plot = 2; %for the plotting of the vertical axis (not the scale)
                plot([0 0],[ymin_plot ymax_plot],'color',axislcolor); % draw vert axis at time 0  
                axis('off');
                ...
                %
                vert_ticks_temp=round(xmin/100)*100:100:round(xmax/100)*100; 
                g.vert=[xmin vert_ticks_temp(2:end-1) xmax]; %mark every 100 ms but don't round the ends (so you end up with something looking like [-199, -100, 0, 100, 200, 300, 400, 500, 598]
                %if isempty(g.vert)
                %    g.vert = [xmin xmax];
                %    ymean = (ymin+ymax)/2; 
                ...
                %end
                for v = g.vert
                    plot([v v],[.5*ymin_plot .5*ymax_plot],'color',vertcolor); % draw vertical lines 
                end
                ...
                %
                if isempty(g.hori)
                    g.hori = [ymin_plot ymax_plot]; 
                end
                if ~isnan(g.hori)
                    xmean = 0; 
                    hmin = xmean-0.1*(xmean-xmin);
    ...
    ax = axes('Units','Normal','Position', ...
              [0.0845 0.155 axwidth axheight]); % FIX!!!!
    axes(ax)
    axis('off');
    if xmin <=0
        figure(curfig);p=plot([0 0],[ymin_plot ymax_plot],'color','k'); % draw vert axis at zero
    else
        figure(curfig);p=plot([xmin xmin],[ymin_plot ymax_plot],'color','k'); % draw vert axis at zero
    end
    if g.ydir == -1
        set(gca, 'ydir', 'reverse');
    end;
    axis([xmin xmax ymin_plot ymax_plot]);        % set axis values
    hold on
    ...
    if ~isnan(g.vert)
        for v = g.vert
            figure(curfig);plot([v v],[.5*ymin_plot .5*ymax_plot],'color',vertcolor); % draw vertical lines 
        end
    end
    %
    %%%%%%%%%%%%%%%%%%%% plot horizontal lines (optional) %%%%%%%%%%%%%%%%%
    ...
    figure(curfig);axis('off');h=text(double(signx),double(ymin_plot),[num2str(ymin_plot,3) 'µV']); % text ymin
    set(h,'FontSize',TICKFONTSIZE);               % choose font size
    set(h,'HorizontalAlignment','center','Clipping','off');

    %figure(curfig);axis('off');h=text(double(signx), double(ymax_plot),['+' num2str(ymax_plot,3) 'µV']);  % text +ymax
    %set(h,'FontSize',TICKFONTSIZE);         % choose font size
    %set(h,'HorizontalAlignment','right','Clipping','off');

    ytick = g.ydir*(-ymax_plot-0.2*ydiff);
    figure(curfig);tick = [int2str(round(xmin/100)*100)]; h=text(double(xmin),double(ytick),tick);
    set(h,'FontSize',TICKFONTSIZE);         % choose font size
    set(h,'HorizontalAlignment','center',...
          'Clipping','off');  % center text

    tick = [xlabel]; figure(curfig);h=text(double(xmin+xdiff/2),double(ytick-0.15*g.ydir*ydiff),tick);
    set(h,'FontSize',TICKFONTSIZE);         % choose font size
    set(h,'HorizontalAlignment','center',...
          'Clipping','off');  % center text

    tick = [int2str(round(xmax/100)*100)]; figure(curfig);h=text(double(xmax),double(ytick),tick);
    set(h,'FontSize',TICKFONTSIZE);         % choose font size
    set(h,'HorizontalAlignment','center',...
          'Clipping','off');  % center text
    ...
    %sh = subplot(20, 20, 390);
    %p  = get(sh,'position');
    %h_legend = legend('String1','String2','String3','String4','Location','Best');
    %h_legend = legend(sh, [400 410] );
