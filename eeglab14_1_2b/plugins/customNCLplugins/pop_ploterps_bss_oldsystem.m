...
function [ERP, erpcom] = pop_ploterps_bss_oldsystem(ERP, binArray, chanArray, varargin)
erpcom = '';
if nargin < 1
        help pop_ploterps
        return
...
if nargin==1  %with GUI
        ...
        % Somersault
        %
        [ERP, erpcom] = pop_ploterps_bss_oldsystem(ERP, binArray, chanArray, 'AutoYlim', rAutoYlim, 'BinNum', rBinNum, 'Blc', blcorr,...
...
p.addParamValue('LineWidth', 3, @isnumeric);
...
elseif strcmpi(p.Results.Style,'Topo')
        qpstyle = 4;
elseif strcmpi(p.Results.Style,'Gina')
        qpstyle = 5;
else
...
if qpstyle==4 || qpstyle==5 % topo or Gina
        if ~isfield(ERP.chanlocs,'theta')
...
ploterps_bss_oldsystem(ERP, binArray, chanArray,  qpstyle, qMgfp, qBlc, qxscale, qyscale, qLineWidth, qisiy, qFontSizeChan, qFontSizeLeg, qFontSizeTicks, qerrorstd,...
